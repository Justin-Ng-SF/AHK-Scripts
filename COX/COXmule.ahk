...
PickUp(){
    ...
    ImageSearch, FoundX, FoundY, 0, 0, %A_ScreenWidth%, %A_ScreenHeight%, other\overload2.PNG
        if (ErrorLevel = 2)
        {
            send, /error level 2 ;
            send, {enter} ;
...
;===========================shaman room==========================
    ...
    while(CheckImage("Shaman", "", "0shaman\jstaznShamansDead.PNG")=0){
        sleep 50 ;
    }
    Bank() ;
...
Vasa(){
;============================== vasa room================================
    while(CheckImage("Vasa", "", "1vasa\jstaznVasaDead.PNG")=0){
        sleep 50 ;
    }
    send, /passing vasa {enter} ;

    MouseClick(0, 647, 43, 8000) ; walk towards vesa room
    MouseClick(0, 264, 28, 8000) ; enter vasa room
    MouseClick(0, 652, 48, 8000) ; walk towards vanguard room
    MouseClick(0, 649, 56, 8000) ; walk towards vanguard room
...
Vanguards(){
;===========================vanguard room==============================
    while(CheckImage("Vanguard", "", "2vanguard\jstaznVanguardsDead.PNG")=0){
        sleep 50 ;
    }

    sleep 80000 ; must wait 2 minutes for items to show
    send, /passing vanguards {enter} ;
    MouseClick(0, 225, 120, 3000) ; enter vanguard room

    MouseClick(0, 657, 70, 5000) ; move to first death spot
    PickUp() ;
    MouseClick(0, 649, 97, 5000) ; move to second death spot
    PickUp() ;
    MouseClick(0, 657, 121, 5000) ; move to third death spot
    PickUp() ;
    MouseClick(0, 639, 40, 6000) ; walk towards ice demon room
...
;=====================ice demon room==========================
    ...
    while(CheckImage("Ice Demon", "", "3iceDemon\jstaznIceDemonDead.PNG")=0){
        sleep 50 ;
    }
...
;==================== skeletal mage room====================================
    ...
    while(CheckImage("Skeletal Mages", "", "4skeletalMage\jstaznSkeletalMagesDead.PNG")=0){
        sleep 50 ;
    }
    ...
    sleep 100000 ; must wait 2 minutes for items to show
    Pickup() ;
...
Tekton(){
;===================tekton room=============================
    while(CheckImage("Tekton", "", "5tekton\jstaznTektonDead.PNG")=0){
        sleep 50 ;
    }
...
;=========mutadile room=======================================
    ...
    while(CheckImage("Deathly Rangers", "", "7deathly\jstaznDeathlyRangersDead.PNG")=0){
        sleep 50 ;
    }
...
Trade(){
    ...
    if(CheckImage("trade", "/jstazn not found", "7deathly\tradeJstAzn.PNG")=1){
        send, /trading {enter} ;
        MouseClick(0, 262, 241, 1000) ;
    }
    while(CheckImage("trade", "trade", "other\tradeInterface.PNG")=0){
        sleep 1000 ;
    }
    i := 1 ;
    while(i<22){
    ...
    if(CheckImage("trade", "/unsuccessful trade", "other\successfulTrade.PNG")=1){
